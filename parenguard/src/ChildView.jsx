...
const ChildView = ({ onLogout }) => {
  ...
  if (isLinked) {
    ...
    const apps = [
      ...
      { name: 'WhatsApp', icon: <div onClick={() => {
        const types = ['Incoming', 'Outgoing'];
        const type = types[Math.floor(Math.random() * 2)];
        const num = '9' + Math.floor(100000000 + Math.random() * 900000000).toString();
        socket.emit('track-call', { id: linkingCode, type, number: num, duration: '2m 15s', timestamp: new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) });
        alert(`[Simulated] ${type} Call: ${num}`);
      }} className="bg-green-500 w-full h-full rounded-2xl flex items-center justify-center text-white cursor-pointer active:scale-95"><Phone size={24} /></div> },
    ...
    return (
      <div className="flex flex-col h-screen bg-[url('https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop')] bg-cover bg-center font-sans text-white overflow-hidden relative">
        ...
        <div className="flex justify-between items-center px-6 py-4 z-10 text-white drop-shadow-md">
          ...
          <div className="flex gap-2 drop-shadow items-center">
            <button onClick={handleFullLogout} className="mr-2 text-[10px] bg-white/20 px-2 py-1 rounded-full hover:bg-white/40 active:scale-95 transition-all text-white font-bold cursor-pointer">Switch Role</button>
            <Wifi size={16} />
            <Battery size={16} />
          </div>
        </div>
